...
Thanks for your interest in improving @makeitaquote/utils!
...
You'll need Node.js >= 22 to build and test this repo — the dev toolchain (tsdown/rolldown) needs a
newer Node than the package itself does. The published package's own `engines.node` (>=18) is lower,
and backed by the `compat-smoke` CI job, which builds once on Node 22 and then runs
`scripts/check-dist.cjs` against the built `dist/` directly on every Node version it claims to
support — no dev toolchain involved at that point.
...
| `npm run check-dist` | Smoke-test the built `dist/` directly with `require()` — no dev toolchain involved, run after `npm run build` |
